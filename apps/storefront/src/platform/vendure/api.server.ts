...
import { getChannelTokenForRegion } from "../region/region.server.ts";
...
export async function executeVendureRequest<T>({
	...
	const channelToken =
		options?.channelToken !== undefined
	...
	// If custom channel header caused an error (e.g. channel not yet provisioned), fallback to default channel
	if (!response.ok && headers[channelHeader]) {
		const fallbackHeaders = { ...headers };
		delete fallbackHeaders[channelHeader];
