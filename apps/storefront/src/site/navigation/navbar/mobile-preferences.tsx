...
import { LANGUAGES } from "@/site/navigation/navbar/language-picker";
...
/**
 * Language, region, currency and theme controls for the mobile drawer.
 *
...
export function MobilePreferences({
	...
	return (
		<div className="flex flex-col gap-6">
			<PreferenceGroup label="Market & Region">
				{(availableRegions && availableRegions.length > 0
					...
					return (
						<PreferenceOption
							...
							name="mobile-region"
							code={reg.flag}
							label={`${reg.name} (${reg.currencyCode})`}
							selected={isSelected}
							disabled={isPending}
							onSelect={() => {
								startTransition(async () => {
					...
					<PreferenceOption
						...
						selected={
							typeof window !== "undefined"
								? (localStorage.getItem("suisuto_language") || "en") === lang.code
								: lang.code === "en"
						}
						onSelect={() => {
							if (typeof window !== "undefined") {
								localStorage.setItem("suisuto_language", lang.code);
								document.cookie = `suisuto_language=${lang.code};path=/;max-age=31536000`;
							}
							router.refresh();
						}}
					/>
